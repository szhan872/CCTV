...
BOOL __stdcall I8_MP4Play_RigisterDrawFunFast(LONG nPort,I8DRAWFUN DrawFunFast,LONG nUser);

//往流中输入原始数据
//uiFrameTyp的含义 IFrames=0x01,AudioFrames=0x08,PFrames=0x09
//uiFrameNo为帧号，注意视频帧的帧号必须连续，音频帧号无所谓
//uiFrameTime为帧的秒级时间戳，也就是事件发生时候的当前时间
//TimeStamp是相对时间戳，前后两帧的相对时间戳之差/90就是两帧之间的毫秒数
//unsigned short usWidth 视频的宽度
//unsigned short usHeight 视频的高度
